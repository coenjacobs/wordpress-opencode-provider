...
composer:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/opencode-provider && composer install --no-interaction"

mozart:
	docker run --rm -v "$(CURDIR)/plugin:/project" coenjacobs/mozart /mozart/bin/mozart compose
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/opencode-provider && composer dump-autoload"

activate:
	$(WP) plugin activate opencode-provider

test:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/opencode-provider && composer test"

test-lint:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/opencode-provider && composer test:lint"

test-phpstan:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/opencode-provider && composer test:phpstan"

test-phpmd:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/opencode-provider && composer test:phpmd"

test-docs:
	$(DOCKER_EXEC) sh -c "cd /var/www/html/wp-content/plugins/opencode-provider && composer test:docs"
